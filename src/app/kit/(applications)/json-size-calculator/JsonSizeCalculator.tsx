        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-slate-900/80 backdrop-blur-sm rounded-full px-4 py-2 mb-4 shadow-lg border border-slate-800">
              ...
              <span className="text-xs font-medium text-gray-200">Developer Tools</span>
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 shadow-xl">
              ...
              <CardContent className="space-y-4">
                <Textarea
                  ...
                  className="font-mono text-sm border-2 border-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-900 bg-slate-800 text-gray-100 placeholder:text-gray-400 transition-all duration-200"
                />
                
                {/* Stats */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gradient-to-r from-blue-900/80 to-indigo-900/80 p-3 rounded-lg text-center border border-blue-800">
                    <div className="text-lg font-bold text-blue-200">{charLength}</div>
                    <div className="text-xs text-gray-400">Characters</div>
                  </div>
                  <div className="bg-gradient-to-r from-green-900/80 to-emerald-900/80 p-3 rounded-lg text-center border border-green-800">
                    <div className="text-lg font-bold text-green-200">{formatBytes(byteSize)}</div>
                    <div className="text-xs text-gray-400">Size</div>
                  </div>
                </div>

                {isJsObj && (
                  <div className="bg-gradient-to-r from-purple-900/80 to-pink-900/80 p-3 rounded-lg text-center border border-purple-800">
                    <div className="text-lg font-bold text-purple-200">{formatBytes(objectSize)}</div>
                    <div className="text-xs text-gray-400">Object Size (approx)</div>
                  </div>
                )}
                ...
                {(isJson || isJsObj) && (
                  <div className="flex flex-wrap gap-2">
                    {isJson && (
                      <Button variant="outline" size="sm" onClick={handleMinify} className="h-8 border-slate-700 text-gray-200 hover:bg-slate-800">
                        <Minus className="h-3 w-3 mr-1" />
                        Minify
                      </Button>
                    )}
                    <Button variant="outline" size="sm" onClick={handleGenerateInterface} className="h-8 border-slate-700 text-gray-200 hover:bg-slate-800">
                      <Code className="h-3 w-3 mr-1" />
                      Generate TS
                    </Button>
                    {isJsObj && (
                      <Button variant="outline" size="sm" onClick={handleConvertToJson} className="h-8 border-slate-700 text-gray-200 hover:bg-slate-800">
                        <ArrowRight className="h-3 w-3 mr-1" />
                        To JSON
                      </Button>
                    )}
                    {isJson && (
                      <Button variant="outline" size="sm" onClick={handleConvertToJsObject} className="h-8 border-slate-700 text-gray-200 hover:bg-slate-800">
                        <ArrowRight className="h-3 w-3 mr-1" />
                        To JS
                      </Button>
                    )}
                ...
                {error && (
                  <div className="text-red-400 text-sm bg-red-900/40 p-2 rounded-lg border border-red-800">
                    {error}
                  </div>
                )}
              </CardContent>
            ...
            <Card className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 shadow-xl">
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2 text-lg font-semibold text-gray-100">
                  <div className="p-1.5 bg-gradient-to-r from-green-700 to-emerald-800 rounded-lg">
