...
const CashOutCalculator: React.FC<CashOutCalculatorProps> = ({ title, defaultRate, altRate, altLabel }) => {
  ...
  return (
    <div className="p-2">
      <div className="bg-slate-900/95 border border-slate-800 rounded-xl shadow-md p-4 space-y-4 transition-all max-w-md mx-auto">
        <div className="flex items-center gap-2 mb-2">
          <CreditCard className="h-4 w-4 text-blue-400" />
          <span className="font-semibold text-gray-100 text-sm">{title}</span>
          <button
            onClick={handleReset}
            className="ml-auto p-1 rounded hover:bg-slate-800 transition"
            title="Reset"
            type="button"
          >
        ...
        <div className="grid grid-cols-1 gap-2">
          <label className="text-xs text-gray-400 font-medium">Initial Balance</label>
          <input
            type="number"
            min="0"
            value={initialBalance}
            onChange={e => { setInitialBalance(e.target.value); setLastChanged('initial') }}
            className="rounded-md border border-slate-700 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-800 text-gray-100 placeholder:text-gray-400"
            placeholder="0"
          />
        </div>
        <div className="grid grid-cols-1 gap-2">
          <div className="flex items-center justify-between">
            <label className="text-xs text-gray-400 font-medium">Withdraw Amount</label>
            <button
              type="button"
              onClick={handleMaxWithdraw}
              className="px-2 py-1 text-xs rounded-md bg-blue-800 text-blue-100 font-medium hover:bg-blue-700 transition"
            >Max Withdraw</button>
          </div>
          <input
            type="number"
        ...
        <div className="grid grid-cols-1 gap-2">
          <label className="text-xs text-gray-400 font-medium">Closing Balance</label>
          <input
            type="number"
            min="0"
        ...
        <div className="grid grid-cols-1 gap-2">
          <label className="text-xs text-gray-400 font-medium">Receive Cash</label>
          <input
            type="number"
            min="0"
            value={receiveCash}
        ...
        <div className="flex items-center gap-4 mt-1">
          ...
          Charge:
            <input
              ...
              className="w-14 px-1 py-0.5 text-xs border border-blue-800 rounded bg-slate-900 text-blue-300 font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 text-right"
              style={{ minWidth: 0 }}
            />
            <span className="text-xs text-gray-400">%</span>
          </span>
        </div>
        <div className="bg-blue-900/80 border border-blue-800 rounded-lg p-3 mt-2 text-xs space-y-1 shadow-sm transition-all">
